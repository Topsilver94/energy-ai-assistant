...
**所有计算系数一律不写死在组件里**，全部集中在 configStore，由页头三个入口「API 设置 / 专家参数 / 公开平台数据参考」修改，保存后所有依赖该系数的结果即时重算。后两个入口打开**两个独立抽屉**（同一组件按 scope 渲染，一次只开一个）：专家参数抽屉（光伏/储能/集中供冷/充电桩可调系数，scope: expert）与公开平台数据参考抽屉（分省年等效利用小时、分省工商业电价、分省峰谷价差、通用系数、建筑能耗基准、屋面光伏/供冷折算/充电桩配建参考，scope: public；低频校准、常作对外参考展示）。分组契约里 `scope` 字段决定归属。
...
**方案配置推荐（模块①，确定性规则引擎）**：`utils/recommend.js` 依据诊断输入（性质/类型/面积/省份/屋面类型/建成年份）与 config 经验参考表（屋面/供冷折算/充电桩配建，公开数据抽屉可查可调）按 `data/recommendationRules.js` 推断阈值打分排序，输出推荐等级/触发依据/建议规模/置信度；热力图财务列按建议规模调 `calculateFeasibility` 单系统预估。新建建筑不输出节能潜力%（无实际能耗基线，红线：不编造数据）；规则文件只留打分/分桶/门槛/预选类逻辑参数（带 source），不进任何面板。
