...
| 模块 | 输入 | 输出 |
|---|---|---|
| ① 挖掘痛点 | 建筑性质（既有/新建）+ 面积 / 类型 / 省份；既有另填年份与年度电费（另可选填变压器容量，留空按类型配变指标推定，用于储能定容），新建可选填设计能耗强度 | 既有：单位面积能耗对标、节能潜力%、3 条技术建议；新建：设计校核与预估能耗（不输出节能潜力）；两者均输出方案配置推荐（规则引擎）+ 投资价值热力图，可一键填入模块②并自动跳转 |
...
**所有计算系数一律不写死在组件里**，全部集中在 configStore，由页头三个入口「API 设置 / 专家参数 / 公开平台数据参考」修改，保存后所有依赖该系数的结果即时重算。后两个入口打开**两个独立抽屉**（同一组件按 scope 渲染，一次只开一个）：专家参数抽屉（光伏/储能/集中供冷/充电桩可调系数，scope: expert）与公开平台数据参考抽屉（分省年等效利用小时、分省工商业电价、分省峰谷价差、分省分时结构、通用系数、建筑能耗基准、屋面光伏/供冷折算/充电桩配建/储能定容参考，scope: public；低频校准、常作对外参考展示）。分组契约里 `scope` 字段决定归属。
