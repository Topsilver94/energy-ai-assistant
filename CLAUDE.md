...
| 模块 | 输入 | 输出 |
|---|---|---|
| ① 挖掘痛点 | 建筑性质（既有/新建）+ 面积 / 类型 / 省份；既有另填年份与电费（按年/按月口径可切；留空按典型强度或变压器容量双口径取短板预估，结果挂预估标注），另可选填变压器容量（留空按类型配变指标推定，用于储能定容），新建可选填设计能耗强度 | 既有：单位面积能耗对标、节能潜力%、3 条技术建议；新建：设计校核与预估能耗（不输出节能潜力）；两者均输出方案配置推荐（规则引擎）+ 投资价值热力图，可一键填入模块②并自动跳转 |
...
**所有计算系数一律不写死在组件里**，全部集中在 configStore，由页头三个入口「API 设置 / 专家参数 / 公开平台数据参考」修改，保存后所有依赖该系数的结果即时重算。后两个入口打开**两个独立抽屉**（同一组件按 scope 渲染，一次只开一个）：专家参数抽屉（光伏/储能/集中供冷/充电桩可调系数，scope: expert）与公开平台数据参考抽屉（分省年等效利用小时、分省工商业电价、分省峰谷价差、分省分时结构、通用系数、建筑能耗基准、屋面光伏/供冷折算/充电桩配建/储能定容/电耗预估参考，scope: public；低频校准、常作对外参考展示）。分组契约里 `scope` 字段决定归属。
...
- 储能套利：年放电量 = 容量 × 365 ×(1 + 第二循环价差比)，收益 = 放电量 × 分省峰谷价差；循环判定读分省 cyclesPerDay（1=一充一放全额价差，2=两充两放、第二循环按 cycle2SpreadRatio 折价），均为公开数据项
- 节能潜力% = (实际单位能耗 − 基准单位能耗) / 实际单位能耗 × 100%

**方案配置推荐（模块①，确定性规则引擎）**：`utils/recommend.js` 依据诊断输入（性质/类型/面积/省份/屋面类型/建成年份/变压器容量选填）与 config 经验参考表（屋面/供冷折算/充电桩配建/储能定容，公开数据抽屉可查可调）按 `data/recommendationRules.js` 推断阈值打分排序；储能定容双口径取短板（负荷消纳 = 年电量→日均×峰段可转移系数；变压器接入 = 实填或按分类型配变指标推定的容量×功率占比×2h），运行模式按分省分时结构判定（两充两放 8 省→全额套利模式，其余→一充一放+需量管理，名单随 CYCLES_AS_OF 常量换版），新建无负荷数据按光储配比兜底，布置红线（GB/T 51048-2025 等现行国标）双出口注入推荐依据与模块③ prompt，输出推荐等级/触发依据/建议规模/置信度；热力图财务列按建议规模调 `calculateFeasibility` 单系统预估。新建建筑不输出节能潜力%（无实际能耗基线，红线：不编造数据）；规则文件只留打分/分桶/门槛/预选类逻辑参数（带 source），不进任何面板。
