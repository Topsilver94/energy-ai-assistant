...
**所有计算系数一律不写死在组件里**，全部集中在 configStore，由页头三个入口「API 设置 / 专家参数 / 公开平台数据参考」修改，保存后所有依赖该系数的结果即时重算。后两个入口打开**两个独立抽屉**（同一组件按 scope 渲染，一次只开一个）：专家参数抽屉（光伏/储能/集中供冷/充电桩可调系数，scope: expert）与公开平台数据参考抽屉（分省年等效利用小时、分省工商业电价、分省峰谷价差、通用系数、建筑能耗基准、屋面光伏/供冷折算/充电桩配建/储能定容参考，scope: public；低频校准、常作对外参考展示）。分组契约里 `scope` 字段决定归属。
